...
export const UserProvider = ({ children }) => {
  ...
  const logout = (navigate) => {
    // Clear user-specific chat data from localStorage before logout
    const userData = localStorage.getItem('dashboardAgent_user');
    if (userData) {
      try {
        const user = JSON.parse(userData);
        // Clear all user-specific chat data
        localStorage.removeItem(`chatHistory_${user.email || user.id}`);
        localStorage.removeItem(`chartCustomization_${user.email || user.id}`);
        localStorage.removeItem(`pinnedCharts_${user.email || user.id}`);
        console.log('🗑️ Cleared all user chat data on logout');
      } catch (error) {
