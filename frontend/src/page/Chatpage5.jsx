...
const ChatPage = () => {
  ...
  const downloadDashboard = async () => {
    ...
    showNotification('📄 Generating PDF with chart images...', 'info');
    ...
    try {
      ...
      for (let i = 0; i < pinnedCharts.length; i++) {
        ...
        // Add chart title and query
        pdf.setTextColor(44, 62, 80);
        pdf.setFontSize(16);
        pdf.setFont(undefined, 'bold');
        pdf.text(`Chart ${i + 1}: ${chart.data[0]?.[chart.customization.yAxis] || 'Data'} Analysis`, 20, yPosition);
        ...
        // Wrap long query text
        const splitQuery = pdf.splitTextToSize(`"${chart.query}"`, pageWidth - 40);
        pdf.text(splitQuery, 20, yPosition);
        yPosition += splitQuery.length * 5 + 5;
        ...
        if (chartElement) {
          try {
            ...
            const chartDetails = `${chart.data[0]?.[chart.customization.yAxis]} vs ${chart.data[0]?.[chart.customization.xAxis]} (${chart.customization.chartType.charAt(0).toUpperCase() + chart.customization.chartType.slice(1)} Chart)`;
            pdf.text(chartDetails, 20, yPosition);
            yPosition += 10;
            ...
            pdf.text('❌ Chart image could not be captured', 20, yPosition);
            yPosition += 20;
          }
        }
      ...
      pdf.setFontSize(10);
      pdf.setFont(undefined, 'normal');
      pdf.text(`📊 Dashboard contains ${pinnedCharts.length} pinned charts • Generated by Data Analytics Platform`, pageWidth / 2, footerY + 5, { align: 'center' });
      ...
      showNotification('❌ Error generating PDF. Please try again.', 'info');
    } finally {
                      ...
                      <div className="mt-6 p-4 bg-blue-50 rounded-lg border border-blue-200">
                        <div className="flex items-center justify-between">
                          <div className="text-blue-700">
                            ...
                            <p className="text-sm">Click "Generate Report" to download a PDF with actual chart images and data visualizations.</p>
                          </div>
                          <button
                            onClick={downloadDashboard}
                            disabled={isGeneratingPDF}
