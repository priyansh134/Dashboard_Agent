...
const ChatPage = () => {
  ...
  const initializeChatRecognition = (conversationId) => {
    if ('webkitSpeechRecognition' in window || 'SpeechRecognition' in window) {
      ...
      recognitionInstance.onend = () => {
        ...
        // Don't auto-restart here - let the speech end callback handle it
        // This prevents multiple restart attempts
      };
      ...
      recognitionInstance.onerror = (event) => {
        ...
        if (shouldRetry) {
          console.log('🔄 Retrying voice recognition after recoverable error:', event.error);
          setTimeout(() => {
  ...
  // Setup interrupt detection while AI is speaking
  const setupInterruptDetection = (conversationId) => {
    if (!('webkitSpeechRecognition' in window || 'SpeechRecognition' in window)) {
      return;
    }

    console.log('🎙️ Setting up interrupt detection for conversation:', conversationId);
    ...
    interruptInstance.onstart = () => {
      console.log('🎙️ Interrupt detection started');
    };
    ...
    interruptInstance.onresult = (event) => {
      ...
      for (let i = 0; i < event.results.length; i++) {
        ...
        // Only trigger on final results with decent confidence to avoid false triggers
        if (result.isFinal && result[0].confidence > 0.5) {
          console.log('🛑 User speech detected! Interrupting AI...', result[0].transcript);
          interruptAI(conversationId);
          break;
        }
        // Also trigger on strong interim results for faster response
        else if (!result.isFinal && result[0].confidence > 0.7 && result[0].transcript.length > 2) {
          console.log('🛑 Strong interim speech detected! Interrupting AI...', result[0].transcript);
          interruptAI(conversationId);
          break;
        }
      }
    ...
    interruptInstance.onerror = (event) => {
      console.log('🎙️ Interrupt detection error (normal):', event.error);
      // Don't restart on error - this is just for detection
    };
    ...
    try {
      ...
      console.log('🎙️ Could not start interrupt detection:', error);
    }
  };
  ...
  const interruptAI = (conversationId) => {
    console.log('🛑 Interrupting AI for conversation:', conversationId);
      ...
      if (response.data.success) {
        ...
        setTimeout(() => {
          speakChatResponse(conversationId, response.data.answer, () => {
            ...
            console.log('🔄 Speech ended, checking if should restart listening...');
            if (continuousMode[conversationId]) {
              setTimeout(() => {
              ...
              }, 800); // Wait a bit longer for better flow
            }
          });
        }, 100); // Start speech sooner
    ...
    const speakWithVoices = () => {
      try {
        ...
        utterance.onstart = () => {
          ...
          // Start interrupt detection when AI begins speaking
          if (continuousMode[conversationId]) {
            setTimeout(() => {
              setupInterruptDetection(conversationId);
            }, 500); // Small delay to avoid immediate false triggers
          }
        };
                              ...
                              <div className="flex items-center justify-between mb-4">
                                ...
                                {continuousMode[conversation.id] && (
                                  <div className="flex items-center space-x-3">
                                    ...
                                    {/* Auto-interrupt indicator (no button needed) */}
