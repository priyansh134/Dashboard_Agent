...
try:
    # Primary connection attempt with proper SSL parameters
    client = MongoClient(
        mongodb_uri,
        tls=True,
    ...
    print(f"❌ Primary Atlas connection failed: {ssl_error}")
    print("🔄 Trying fallback connection with relaxed SSL...")
    try:
        # Fallback connection for SSL handshake issues
        import ssl
        client = MongoClient(
            mongodb_uri,
            ssl=True,
        ...
        db = client[database_name]
        users_collection = db.users
        print("✅ Connected to MongoDB Atlas with fallback SSL settings")
    except Exception as fallback_error:
        print(f"❌ Fallback connection also failed: {fallback_error}")
        print("💡 Consider using a local MongoDB or check your Atlas configuration")
        db = None
